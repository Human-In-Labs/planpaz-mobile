...
export default function LocationOverlay({
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(
            async () => {
                try {
                    const data = await locationService.search(term);
                    setLocations(data);
                } catch (error) {
                    console.error(
    ...
    const containerStyle = hasResults
        ? styles.expandedContainer
        : styles.compactContainer;

    const handleSelect = (location: LocationSearchData) => {
        onSelect(location);
        setSearch('');
        setLocations([]);
        onClose();
        ...
        >
            ...
            {hasResults && (
                <FlatList
                    ...
                    renderItem={({ item }) => {
                        const neighborhood = splitHighlight(
                            item.neighborhood,
                            search,
                        );

                        const city = splitHighlight(
                            item.city,
                            search,
                        );

                        const state = splitHighlight(
                            item.state,
                            search,
                        );
